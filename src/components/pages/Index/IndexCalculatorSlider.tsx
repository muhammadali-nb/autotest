    ...
    return (
        <div className={'mb-px-10 ' + (props.className ?? '')} style={props.style}>
            <div className={'d-flex justify-content-between align-items-center ' + (props.wideSpace ? 'mb-px-10' : '')}>
                <span className={'indexCalculatorSliderLabel ' + (props.labelClass ?? '')}
                      style={props.labelStyle}>{props.label}</span>
                <div className={'indexCalculatorSliderValue ' + (props.valueClass ?? '')}>
                    <span className={(props.valuePrefixClass ?? '')}
                          style={props.valuePrefixStyle}>{props.valuePrefix}</span>
                    <span style={props.valueStyle}>{processValue(props.value ?? 0)}</span>
                    <span className={(props.valueSuffixClass ?? '')}
                          style={props.valueSuffixStyle}>{props.valueSuffix}</span>
                </div>

            </div>
            <div className={'w-100'}>
                <Slider max={props.max} min={props.min} value={props.value} step={props.step}
