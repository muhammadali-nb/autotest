...
const IndexCalculatorPeriodIndicator: React.FC<{
    ...
    return (<div className={'indexCalculatorPeriodIndicator' + (props.right ? ' text-end' : '')
        ...
        <span
            className={'indexCalculatorPeriodIndicatorComment' + (props.bold ? ' font-weight-medium' : '')}>{props.comment ?? ''}</span>
    </div>)
}
...
const IndexCalculator: React.FC<IndexCalculatorProps> = (props) => {
    ...
    return (
        <div
            className={'d-flex align-items-center justify-content-center flex-column w-100' + (props.noAnim ? '' : ' anim-enter-bottom-2')}
            id={'logo_calc'}>
            {!props.hidePrice && <IndexCalculatorSlider wideSpace={props.wideSpace}
                                                        className={'calculator-slider w-100 '}
                                                        max={props.maxPrice ?? 10000000}
                                                        min={props.minPrice ?? 1500000} step={props.priceStep ?? 50000}
                                                        value={values.price}
                                                        label={'Стоимость автомобиля'} valueSuffix={' ₽'}
                                                        onChange={(e) => handleUpdate({price: e})}/>}
            <IndexCalculatorSlider wideSpace={props.wideSpace}
                                   className={'w-100 calculator-slider ' } min={values.minPrepaid}
                                   max={values.localMaxPrepaid}
                                   step={props.prepaidStep ?? 5000} value={values.prepaid}
            ...
            />
            <IndexCalculatorSlider wideSpace={props.wideSpace}
                                   className={'w-100  calculator-slider' } min={props.minTime ?? 3}
            ...
            <div className={'d-flex w-100 justify-content-between'}>
                <IndexCalculatorPeriodIndicator amount={daySum()} suffix={" ₽"} comment={" в сутки"}
                                                bold={props.fontBold}/>
                <IndexCalculatorPeriodIndicator amount={monthSum()} suffix={" ₽"} comment={" в месяц"}
                                                bold={props.fontBold}/>
            </div>
        </div>
    );
