...
export const AlertMessage:React.FC<{page:string, type: string, className?:string }> = (props) => {
    ...
    return (
        <div className={'_alert ' + props.type + (show ? " " : " hidden " ) + (props.className ?? '')}>
            <img src={dangerBtn} alt={''} className={'_alert__image'} />
            <div className={'_alert__content'} dangerouslySetInnerHTML={{
...
export const BottomMessage:React.FC<{ text1: string|ReactNode,text2:string|ReactNode, button:ReactNode }> = (props) => {
    return (
        <div className={'px-px-40 py-px-25 my-px-20 text-white d-flex justify-content-between align-items-bottom'}
             style={{backgroundImage:`url('${bg}')`,backgroundPosition:'center',borderRadius:'2px'}}>
            <div>
                <div className={'font-size-20 font-weight-medium mb-px-15 line-height-120'}>
                    {props.text1}
                </div>
                <div className={'font-size-32 font-weight-semibold text-uppercase line-height-120'}>
                    {props.text2}
                </div>
            </div>
