...
const CarRentContacts:React.FC<{closeFunc:()=>void, setStep:(string) => void, car:CarRentDataInfo,
        setData:(CallRequestData)=>void, data:CallRequestData, closeOnBack?:boolean}> = (props) => {
    ...
    const send = () => {
        ...
        Api.carRentCodeRequest(props.data)
            .then(resp => {
                if(Api.isError(resp)){
                    props.setData({...props.data, errors:{server:'Ошибка соединения с сервером!'}})
                    return;
                }
        ...
        <ModalTemplateContent>
            <div>
                <div className={'mb-px-90'}>
                    <button className={'default-link font-size-18 font-weight-semibold text-hover-default'}
                            onClick={()=>{props.closeOnBack ? props.closeFunc() : props.setStep('rent')}}>
                        <FontAwesomeIcon icon={faAngleLeft} />&nbsp;&nbsp;ВЕРНУТЬСЯ
                    </button>
                </div>
                <div>
                    <div className={'call-content-text-header font-size-40 line-height-120 mb-px-10'}>
                        Бронирование<br/>
                        {brand} {model}
                    </div>
            ...
            <div>
                <div>
                    <ModalTemplateInput error={props.data.errors['name']}
                                        onInput={(e:any)=>update('name', e.target.value)}
                                        onChange={(e:any)=>update('name', e.target.value)}
                                        value={props.data.name}
                                        placeholder={'Имя'}/>
                    <ModalTemplateInput error={props.data.errors['lastName']} onInput={(e:any)=>update('lastName', e.target.value)}
                                        onChange={(e:any)=>update('lastName', e.target.value)}
                                        value={props.data.lastName}
                                        placeholder={'Фамилия'}/>
                    <ModalTemplatePhone error={props.data.errors['phone']} onInput={(e:any)=>update('phone', e.target.value)}
                                        value={props.data.phone}
                                        onChange={(e:any)=>update('phone', e.target.value)}/>
                </div>
                {props.data.errors['server'] && <div className={'my-2 text-red-color font-size-12'}>{props.data.errors['server']}</div>}
            </div>

            <div>
                <button className={'site-btn small ' + (!passed ? 'dark' : '')} onClick={()=>send()}>Отправить код</button>
                <ModalTemplateConfirm error={props.data.errors['confirm']} confirmed={props.data.confirm} onChange={(e)=>update('confirm', e.target.checked)}/>
            </div>
        </ModalTemplateContent>
    )
}
...
const CarRentConfirmPhone:React.FC<{closeFunc:()=>void, setStep:(string) => void, car:CarRentDataInfo, data:CallRequestData}> = (props) => {
    const [passed,setPassed] = useState(false);
    ...
    const send = () => {
        ...
        Api.carRentConfirmRequest(props.data, code)
            .then(resp => {
                if(Api.isError(resp)){
                    setError("Ошибка соединения с сервером!");
                    return;
                ...
                if(resp.success) {
                    ...
                    setError((resp.fields && resp.fields['code']) ? resp.fields['code'] : 'Неправильный код подтверждения');
                    setPassed(false);
                }
            })
    }
    ...
    return (
        <ModalTemplateContent>
            <div>
                <div className={'mb-px-90'}>
                    <button className={'default-link font-size-18 font-weight-semibold text-hover-default'} onClick={()=>props.setStep('start')}>
                        <FontAwesomeIcon icon={faAngleLeft} />&nbsp;&nbsp;ВЕРНУТЬСЯ
                    </button>
                </div>
                <div>
                    ...
                    <div className={'call-content-text font-size-16'}>
                        <span className={'text-default'}>Мы отправили вам код</span><br/>
            ...
            <div>
                <div className={'d-flex justify-content-between'}>
                    <ModalTemplateInput id={'confirm' + props.car.id + idPrefix + '-0'}
                        container_style={{maxWidth:'40px'}} maxLength={1}
                        onInput={(e:any)=>update(0, e.target.value)}/>
                    <ModalTemplateInput id={'confirm' + props.car.id + idPrefix + '-1'}
                                        container_style={{maxWidth:'40px'}} maxLength={1}
                                        onInput={(e:any)=>update(1, e.target.value)}/>
                    <ModalTemplateInput id={'confirm' + props.car.id + idPrefix + '-2'}
                                        container_style={{maxWidth:'40px'}} maxLength={1}
                                        onInput={(e:any)=>update(2, e.target.value)}/>
                    <ModalTemplateInput id={'confirm' + props.car.id + idPrefix + '-3'}
                                        container_style={{maxWidth:'40px'}} maxLength={1}
                                        onInput={(e:any)=>update(3, e.target.value)}/>
                    <ModalTemplateInput id={'confirm' + props.car.id + idPrefix + '-4'}
                                        container_style={{maxWidth:'40px'}} maxLength={1}
                                        onInput={(e:any)=>update(4, e.target.value)}/>
                    <ModalTemplateInput id={'confirm' + props.car.id + idPrefix + '-5'}
                                        container_style={{maxWidth:'40px'}} maxLength={1}
                                        onInput={(e:any)=>update(5, e.target.value)}/>
                </div>
                {timer > 0 && <div className={'my-px-10 font-size-14 text-gray-color'}>
                  Вы сможете запросить СМС через {timerToString()}
                </div>}
                {timer <= 0 && <div className={'my-px-10'}>
                  <button className={'default-link text-default text-decoration-underline font-size-14'}
                    onClick={requestCode}
                  >Отправить СМС ещё раз</button>
                </div>}
                {error && <div className={'my-2 text-red-color font-size-14'}>{error}</div>}
            </div>

            <div className={'d-flex justify-content-between'}>
                <button className={'site-btn small ' + (!passed ? 'dark' : '')} onClick={()=>send()}>Подтвердить код</button>
                <button className={'default-link text-decoration-none default-transition text-gray-color text-hover-default'}
                    onClick={()=>props.setStep('start')}
                >
                    <FontAwesomeIcon icon={faAngleLeft} />&nbsp;&nbsp;&nbsp;Изменить номер
                </button>
            </div>
        </ModalTemplateContent>
    )
...
const CarRentPaymentButton:React.FC<{payment:string, error:boolean, image:string, text:string, code:string, setPayment:(string)=>void}>
    = ({payment, error,image, text, code, setPayment}) =>{

    return (
        <button className={'car-rent-modal-btn ' + (payment === code ? ' selected' : '') + (error ? ' error' : '')}
...
const CarRentPaymentType:React.FC<{closeFunc:()=>void, data:CallRequestData, setStep:(string) => void, car:CarRentDataInfo}> = (props) => {
    ...
    const send = () => {
        ...
        setRedButton(false);
        Api.carRentPaymentRequest(props.data, props.car, payment)
            .then(resp => {
                if(Api.isError(resp)){
                    setError('Ошибка соединения с сервером')
                    return;
    ...
    return (
        <ModalTemplateContent>
            <div>
                <div className={'mb-px-90'}>
                    <button className={'default-link font-size-18 font-weight-semibold text-hover-default'} onClick={()=>props.setStep('confirm')}>
                        <FontAwesomeIcon icon={faAngleLeft} />&nbsp;&nbsp;ВЕРНУТЬСЯ
                    </button>
                </div>
                <div className={'mb-px-20'}>
                    ...
                    <div className={'font-size-16 line-height-140 font-weight-medium mb-px-40'}>
                        Выберите форму оплаты
                    </div>
                    <div className={'text-default font-size-16 line-height-140 font-weight-semibold mb-px-5'}>
                        К оплате:
                    </div>
                    <div className={'text-default font-size-32 line-height-140 font-weight-semibold'}>
                        {props.car.rentpay} ₽
                    </div>
                </div>
            </div>
            <div>
                <div className={'d-flex flex-column w-100 gap-y-px-15'}>
                    <CarRentPaymentButton payment={payment} error={redButton} image={sbpImg} text={'СБП'} code={'sbp'} setPayment={update} />
                    <CarRentPaymentButton payment={payment} error={redButton} image={bankCardImg} text={'Банковская карта'} code={'card'} setPayment={update} />
                </div>
                <div className={'my-2 text-red-color font-size-12'}>{error || <>&nbsp;</>}</div>
            </div>

            <div>
                <button className={'site-btn small ' + (!passed ? 'dark' : '')} onClick={()=>send()}>Перейти к оплате</button>
            </div>
        </ModalTemplateContent>
    )
}
    ...
    return (
        <ModalTemplateContent>
            <div style={{marginTop:"130px"}}>
                ...
                <div style={{width:'100px',height:'6px',margin:'20px 0'}} className={'bg-red-color'}></div>
            </div>
            <div>
                <button className={'site-btn small'} onClick={()=>props.closeFunc()}>Закрыть</button>
            </div>
        </ModalTemplateContent>
    )
}
...
const CarRequestFormContent:React.FC<{closeFunc:()=>void, setStep:(string) => void, car:CarRentDataInfo}> = (props) => {
    const baseData:BaseState = useAppSelector(state => state.baseData);
    const brand = baseData.left?.brands.values?.find((i) => props.car.brand === i.id)?.name ?? 'неизвестно';
    const model = baseData.left?.models.values?.find((i) => props.car.model === i.id)?.name ?? 'неизвестно';
    const tags = baseData.top?.rent.values?.filter((i) => props.car.special.includes(i.id)) ?? [];
        ...
        <div className={'py-px-35 px-px-60 flex-grow'}>
            <div>
                ...
                <div className={'car__card-taglist mb-px-60'}>
                    <CarTag type={props.car.available ? 'free' : 'not-free'} car={props.car}>{props.car.available ? 'Свободна' : 'Занята'}</CarTag>
                    {tags.map((i, index)=>(<CarTag key={index} car={props.car}>{i.name}</CarTag>))}
                </div>
                ...
                <div className={'mb-px-40'}>
                    <button className={'site-btn big'} onClick={()=>{props.setStep('start')}}>Забронировать</button>
                </div>
...
const CarRequestFormImage:React.FC<{closeFunc:()=>void, car:CarRentDataInfo}> = (props) => {
    ...
    return (
        <div className={'d-flex flex-column position-relative justify-content-center'}
            ...
            <CarImagesModal show={showModal} car={car} handleClose={handleClose} index={index} />
...
const CarBookingForm:React.FC<{wide?:boolean,text?:string|ReactNode, func?:()=>void,
    btn?:ReactNode,step?:string,
    car:CarRentDataInfo}> = (props) => {
    const [show, setShow] = useState(false);
    const [step, setStep] = useState(props.step ?? 'rent');
    const [data, setData] = useState<CallRequestData>({name:'',lastName:'',phone:'', confirm:false,errors:{}});
    ...
    return (
        <>
            <div onClick={handleShow} style={{cursor:'pointer'}}>
                {props.btn ?? <button className={'site-btn big' + (props.wide ? ' w-100':'')}>
                    {props.text ?? <>Забронировать</>}</button>}
            </div>

            <ModalFormTemplate show={show} onHide={handleClose} centered size={'xl'}
                image={step==='rent' ? <CarRequestFormImage closeFunc={handleClose} car={props.car}/> : undefined}
            >

                {(step==='rent') &&
                  <CarRequestFormContent setStep={setStep} closeFunc={handleClose} car={props.car}/>}
                {(step==='start') && <CarRentContacts data={data} setData={setData} closeOnBack={props.step =='start'}
                                          car={props.car} closeFunc={handleClose} setStep={setStep} />}
                {(step==='confirm') && <CarRentConfirmPhone data={data} car={props.car} closeFunc={handleClose} setStep={setStep} />}
                {(step==='payment') && <CarRentPaymentType data={data} car={props.car} closeFunc={handleClose} setStep={setStep} />}
