...
export const CarSameLink: React.FC<{
    ...
    = ({car, style, className, text = 'Посмотреть похожие модели', responsive}) => {
    return (
        <Link to={`/catalog?same=${car.id}`} style={style} className={!responsive ? 'car__card-same  ' : 'car__card-mobile-same  ' + (className ?? '')}>
            <span>{text}</span>
...
export const CarTag: React.FC<{
    ...
    return (
        <div className={` ${small ? 'car__card-mobile-tag' : "car__card-tag"} ` + type + '' + (className ?? '')}
             style={style}>
            {children}
        </div>
    );
...
const CarCard: React.FC<{
    ...
    return (
        <div className={` ${responsive ? 'car__card-mobile' : "car__card"} anim-start-top-4 anim-duration-1800`}
             id={id}>
            <div>
                <div className={` ${responsive ? 'car__card-mobile-taglist' : "car__card-taglist"}  `}>
                    {tags.map((i, index) => (<CarTag key={index} car={car}>{i.name}</CarTag>))}
                </div>

                <Link to={`/catalog/${car.id}`}
                      className={`${responsive ? 'car__card-mobile-image' : 'car__card-image'}`}>
                    <img src={car.thumb} alt={brand + ' ' + model}/>
                </Link>
                <div className={` ${responsive ? 'car__card-mobile-title' : 'car__card-title'} `}>
                    {brand} <span className={'model'}>{model}</span>
                </div>
                <div className={` ${responsive ? 'car__card-mobile-payment' : 'car__card-payment'} `}>
                    Минимальный платеж
                    <div
                        className={` ${responsive ? 'car__card-mobile-payment' : 'car__card-payment-value'} `}>{car.pay.toLocaleString()} ₽</div>
                </div>
                <div className={`${responsive ? 'car__card-mobile-price' : "car__card-price "} `}>
                    Цена от&nbsp;
                    <span
                        className={`${responsive ? 'car__card-mobile-price-value' : "car__card-price-value "}  `}>{car.price.toLocaleString()} ₽</span>
                </div>
                <CarSameLink responsive={responsive} car={car} className={'mb-px-30 font-weight-semibold'}/>
            </div>
            <div className={`${responsive ? 'car__card-mobile-preorder' : "car__card-preorder "} `}>
        ...
        <div className={'car__card'}>
            <div>
                <div className={'car__card-taglist'}>
                    <CarTag type={car.available ? 'free' : 'not-free'}
                            car={car}>{car.available ? 'Свободна' : 'Занята'}</CarTag>
